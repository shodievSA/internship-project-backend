...
class TimeTrackingService {
    ...
    public async startTimer(
		...
		try {
			...
			if (runningTimer) throw new AppError("Timer is already running", 409, true);
    ...
    public async stopTimer(userId: number, note?: string) {
		...
		try {
			...
			if (!runningTimer) {
				...
				throw new AppError("No running timer found to stop", 404, true);
    ...
    public async createManualEntry(
		...
		try {
			...
			if (endTime <= startTime) {
				...
				throw new AppError("End time must be after start time", 400, true);
    ...
    public async updateManualEntry(userId: number, entryId: number, updates: {
		...
		try {
			...
			if (!timeEntry) {
				...
				throw new AppError("Time entry not found or not editable", 404, true);
    ...
    public async deleteManualEntry(userId: number, entryId: number) {
		...
		try {
			...
			if (!timeEntry) {
				...
				throw new AppError("Time entry not found or not deletable", 404, true);
